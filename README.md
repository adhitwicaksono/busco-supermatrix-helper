...
```text
<sample_name>_BUSCO_complete_renamed.fasta → MAFFT
```

because the file contains many unrelated BUSCO genes from the same sample.

Instead, BUSCOs must first be grouped by BUSCO ID across samples (done by the grouping tool provided here).
...
### Why does my tree show BUSCO IDs instead of sample names?
...
```text
<sample_name>_BUSCO_complete_renamed.fasta → MAFFT → tree
```
...
Wicaksono, A., & Purwestri, Y. A. (2026). BUSCO Supermatrix Helper. GitHub repository: https://github.com/adhitwicaksono/busco-supermatrix-helper
